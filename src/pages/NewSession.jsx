...
export default function NewSessionPage() {
  ...
  function updateSetReps(idx, reps) {
    ...
    setSets((prev) => {
      const copy = [...prev];
      const s = { ...copy[idx] };
      if (!Number.isFinite(n) || n < 1) return copy; // let user finish typing
      s.reps = n;
      s.rpe = Array.from({ length: n }, (_, i) => s.rpe[i] ?? Number(s.rpeTemplate));
      s.rep_times_sec = Array.from({ length: n }, (_, i) => s.rep_times_sec[i] ?? Number(s.timeTemplate));
      copy[idx] = s;
      return copy;
    });
  }
  ...
  function updateArrayValue(idx, key, i, val) {
    setSets((prev) => {
      ...
      const arr = [...s[key]];
      const n = Number(val);
      arr[i] = Number.isFinite(n) ? n : arr[i];
      s[key] = arr;
      copy[idx] = s;
      return copy;
    });
  ...
  function fillDefaults(idx, key) {
    setSets((prev) => {
      const copy = [...prev];
      const s = { ...copy[idx] };
      if (key === "rpe") s.rpe = Array.from({ length: s.reps }, () => Number(s.rpeTemplate));
      if (key === "rep_times_sec") s.rep_times_sec = Array.from({ length: s.reps }, () => Number(s.timeTemplate));
      copy[idx] = s;
      return copy;
    });
  }
  ...
  const canSave = useMemo(() => {
    ...
    return sets.every(
      (s) =>
        Number(s.distance_m) > 0 &&
        Number(s.reps) > 0 &&
        Number(s.interval_sec) > 0 &&
        Array.isArray(s.rpe) &&
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 mx-auto max-w-5xl space-y-6">
        {/* Session meta */}
        ...
        <div className="space-y-6">
          {sets.map((s, idx) => (
            <div key={idx} className="rounded-2xl border border-zinc-200 dark:border-zinc-800 p-4">
              ...
              {/* compact controls; add real gaps so nothing overlaps */}
              <div className="grid grid-cols-1 md:grid-cols-6 gap-4 mt-3">
                ...
                />
                <NumSmooth label="Distance (m)" value={s.distance_m} onCommit={(v) => updateSet(idx, { distance_m: v })} min={1} />
                <NumSmooth label="Reps" value={s.reps} onCommit={(v) => updateSetReps(idx, v)} min={1} />
                <NumSmooth
                  label="Interval (sec)"
                  value={s.interval_sec}
                  onCommit={(v) => updateSet(idx, { interval_sec: v })}
                  min={10}
                />
                <div>
                ...
                <NumSmooth label="RPE default" value={s.rpeTemplate} onCommit={(v) => updateSet(idx, { rpeTemplate: v })} min={1} max={10} />
                <NumSmooth
                  label="Time default (sec)"
                  value={s.timeTemplate}
                  onCommit={(v) => updateSet(idx, { timeTemplate: v })}
                  min={10}
                  max={3600}
                />
              </div>
...
function NumSmooth({ label, value, onCommit, min, max }) {
