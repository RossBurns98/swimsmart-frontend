      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {sorted.map((r) => (
            <tr
              key={r.id}
              className="border-b border-zinc-100 dark:border-zinc-900 hover:bg-zinc-50/60 dark:hover:bg-zinc-900/50 cursor-pointer"
              onClick={() => navigate(`/sessions/${r.id}`)}
            >
              <td className="py-2 px-3">{format(new Date(r.date), "dd MMM yyyy")}</td>
              <td className="py-2 px-3">{r.total_distance_m?.toLocaleString()}</td>
              <td className="py-2 px-3">{r.avg_pace_formatted || `${r.avg_pace_sec_per}s / ${r.pace_basis_m}m`}</td>
...
function Th({ label, onClick, active, dir }) {
  return (
    <th onClick={onClick} className="py-2 px-3 select-none">
