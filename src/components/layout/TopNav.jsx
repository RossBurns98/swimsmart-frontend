    ...
    <header className="sticky top-0 z-20 border-b border-zinc-200 dark:border-zinc-800 bg-white/70 dark:bg-zinc-950/70 backdrop-blur">
      <div className="container mx-auto flex h-14 items-center justify-between px-4">
        <div className="flex items-center gap-3">
          <button onClick={goHome} className="text-base font-semibold cursor-pointer">🏊‍♂️ SwimSmart</button>
          {token && (
        ...
        <div>
          {token ? (
            <button className="px-4 py-2 rounded-xl border cursor-pointer" onClick={logout}>Logout</button>
          ) : (
            <button className="px-4 py-2 rounded-xl border cursor-pointer" onClick={() => navigate("/login")}>Login</button>
          )}
        </div>
      </div>
    </header>
